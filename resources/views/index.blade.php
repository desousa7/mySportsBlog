@extends('layouts.app')

@section ('content')
    <div class="background-image grid grid-cols-1 m-auto">
        <div class="flex text-gray-100 pt-10">
            <div class="m-auto pt-4 pb-16sm:m-auto w-3/5 block text-center">
                <h1 class="sm:text-white text-6xl uppercase font-bold text-shadow-md pb-14">
                    About
                </h1>
                <p class="text-center text-white-700 py-2 px-4 font-bold text-xl uppercase">
                    This blog has been designed and created to bring all sports lovers together and create a great
                    community. There will be loads of sports content from around the world and and a chance for us
                    all to share our views and also have some friendly-banter. Feel free to get in touch with me via email 
                    and drop me a follow on instagram.
                </p>
            </div>
        </div>
    </div>
    <script src="{{ asset('/js/notifications.js') }}"></script>
    @yield('js')


@endsection
